...
- **No silent truncation.** Subagent phases that return `skipped-budget`, `skipped-context`, `truncated`, or any equivalent self-imposed-limit marker are hard FAILs of the canonical run. The orchestrator must either re-dispatch the phase to a fresh subagent with a smaller scope (narrower file/project selector, severity filter, or paginated subrange) or record `phase-failed-budget` in the coverage ledger as a P1 audit defect. See the prompt's *Subagent dispatch completion gate* section for the full contract. The same gate applies to inline phases when the host environment cannot spawn subagents.
