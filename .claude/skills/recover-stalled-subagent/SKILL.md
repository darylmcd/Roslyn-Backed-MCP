...
- **`/reconcile-backlog-sweep-plan`** — compatibility-named skill that reconciles `state.json` + `plan.md` against `gh pr view` reality. Runs AFTER recovery, not before. Use this skill first to rescue the worktree and save the WIP to origin; then run `/reconcile-backlog-sweep-plan` to update plan-state with the retry/defer decision (including the STATE_HANDOFF block in `notes`).
