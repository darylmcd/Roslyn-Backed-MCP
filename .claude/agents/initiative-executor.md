...
description: Execute ONE initiative from a remediation plan end-to-end — create the worktree, implement per plan.md, validate, open the PR, return a <<<RESULT>>> envelope. Use proactively when `/backlog-remediate` delegates parallel execution. Orchestrator supplies initiative id, plan path, and tool policy; this agent handles the rest and does NOT merge or touch shared files.
---
