...
public sealed class AnalyzerShadowLoaderLifecycleTests
{
    ...
    public async Task WorkspaceCloseAndReload_ReclaimAnalyzerShadowRoots()
    {
        ...
        Assert.IsTrue(
            WaitForReclamation(workspaceDirectory, surviving: newLeaseDirs[0]),
        ...
        Assert.IsTrue(
            WaitForReclamation(workspaceDirectory, surviving: null, out var closeError),
    ...
    /// </summary>
    private static bool WaitForReclamation(string workspaceDirectory, string? surviving)
        => WaitForReclamation(workspaceDirectory, surviving, out _);
