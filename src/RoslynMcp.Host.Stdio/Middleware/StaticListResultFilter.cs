...
/// <summary>
/// Makes static MCP discovery surfaces deterministic and cacheable. Private scope prevents a
/// shared gateway from reusing one process's tier-filtered surface for another user or server.
/// </summary>
internal static class StaticListResultFilter
{
    ...
    internal static ListToolsResult Normalize(ListToolsResult result)
    {
        ArgumentNullException.ThrowIfNull(result);
        result.Tools = (result.Tools ?? []).OrderBy(static tool => tool.Name, StringComparer.Ordinal).ToArray();
        SetCachingHints(result);
        return result;
    }

    internal static ListPromptsResult Normalize(ListPromptsResult result)
    {
        ArgumentNullException.ThrowIfNull(result);
    ...
    internal static ListResourcesResult Normalize(ListResourcesResult result)
    {
        ArgumentNullException.ThrowIfNull(result);
    ...
    internal static ListResourceTemplatesResult Normalize(ListResourceTemplatesResult result)
    {
        ArgumentNullException.ThrowIfNull(result);
    ...
    private static void SetCachingHints(ICacheableResult result)
    {
