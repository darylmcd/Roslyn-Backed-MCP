...
public static class ScriptingTools
{
    [McpServerTool(Name = "evaluate_csharp", ReadOnly = true, Destructive = false, Idempotent = true, OpenWorld = false),
     Description("Evaluate a C# expression or script interactively using the Roslyn Scripting API. Returns the result value and type. The server enforces a script timeout (default 10 seconds, override per-call with the timeoutSeconds parameter or globally with env ROSLYNMCP_SCRIPT_TIMEOUT_SECONDS — UX-002); infinite loops are cancelled server-side when Roslyn honors the token. If execution exceeds budget plus ROSLYNMCP_SCRIPT_WATCHDOG_GRACE_SECONDS (default 10), a Critical watchdog log fires on a timer (repeat interval ROSLYNMCP_SCRIPT_WATCHDOG_REPEAT_SECONDS, default 60) to flag possible non-cancellable Roslyn work — multi–ten-minute client freezes with no stderr activity usually indicate client/agent or transport issues, not this tool. MCP progress and heartbeats apply as documented. The MCP client may enforce its own session timeout (e.g. error -32001) independently of the server.")]
