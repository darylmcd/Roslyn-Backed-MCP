...
/// </summary>
/// <param name="OverallStatus">One of <c>clean</c>, <c>compile-error</c>, <c>analyzer-error</c>, <c>test-failure</c>, <c>test-zero-run</c>, <c>git-status-unknown</c>, <c>timeout</c>. <c>test-zero-run</c> (added in validate-workspace-runtests-total-zero) indicates <c>runTests=true</c> produced <c>testRunResult.Total=0</c> with a non-empty discovered filter — almost always a filter-resolution failure (working-directory or IChangeTracker timing), not a real pass. <c>git-status-unknown</c> (added in validate-recent-git-changes-status-timeout-false-clean) is emitted only by <see cref="IWorkspaceValidationService.ValidateRecentGitChangesAsync"/> when the <c>git status</c> collection exceeded its own timeout: the verdict would otherwise have been <c>clean</c>, but it was computed over a change-tracker fallback scope rather than the real working tree, so it cannot be distinguished from a genuinely clean tree — re-run (the warning carries <c>retryable=true</c>) or raise <c>ROSLYNMCP_GIT_STATUS_TIMEOUT_SECONDS</c>. <c>timeout</c> is emitted when an internal validation phase exceeds its own timeout: <see cref="CompileCheckDto.Cancelled"/> is <see langword="true"/> and, on <c>runTests=true</c> calls, the test run's failure envelope reports an <c>ErrorKind</c> of <c>Timeout</c>. Callers exact-matching <c>"clean"</c> as the only passing verdict need to treat <c>"test-zero-run"</c>, <c>"git-status-unknown"</c>, and <c>"timeout"</c> as non-passing verdicts too.</param>
