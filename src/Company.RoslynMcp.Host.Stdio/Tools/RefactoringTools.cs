...
public static class RefactoringTools
{
    ...
    public static async Task<string> PreviewRename(
        ...
        [Description("1-based line number of the symbol to rename")] int line,
        [Description("1-based column number of the symbol to rename")] int column,
        [Description("The new name for the symbol")] string newName,
        CancellationToken ct = default)
    {
        var result = await refactoringService.PreviewRenameAsync(filePath, line, column, newName, ct);
    ...
    public static async Task<string> PreviewOrganizeUsings(
        ...
        CancellationToken ct = default)
    {
        var result = await refactoringService.PreviewOrganizeUsingsAsync(filePath, ct);
        return JsonSerializer.Serialize(result, JsonOptions);
    }
    ...
    public static async Task<string> PreviewFormatDocument(
        ...
        CancellationToken ct = default)
    {
        var result = await refactoringService.PreviewFormatDocumentAsync(filePath, ct);
        return JsonSerializer.Serialize(result, JsonOptions);
    }
