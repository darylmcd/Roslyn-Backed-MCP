...
/// <summary>
/// Batch- and first-test-file scaffolding collaborator extracted from <see cref="ScaffoldingService"/>:
/// owns the <c>scaffold_test_batch_preview</c> and <c>scaffold_first_test_file_preview</c> paths.
/// The batch path resolves each target once against a single cached compilation set (one
/// <c>GetCompilationAsync</c> per project, not per target) and aggregates the document adds into a
/// single preview token; the first-test-file path resolves a service by metadata name, infers the
/// destination test project, and renders a brand-new fixture. Content rendering (target resolution,
/// constructor arguments, sibling-pattern extraction, per-framework rendering) is delegated to the
/// shared static <see cref="TestScaffoldRenderer"/> so this collaborator never routes through
/// <see cref="SingleTestScaffolder"/>. Constructed and held privately by the
/// <see cref="ScaffoldingService"/> facade (not DI-registered). The facade's logger-bound
/// resolution helpers (<c>ResolveProject</c> / <c>ValidateIsTestProject</c> /
/// <c>ResolveTestFramework</c>, which surface warnings via the facade's <c>ILogger</c>) are supplied
/// as delegates rather than re-declared here, so the only logging paths stay on the facade.
/// </summary>
internal sealed class BatchTestScaffolder
{
    ...
    /// atomically.
    /// </summary>
    public async Task<RefactoringPreviewDto> PreviewBatchAsync(
    ...
    {
        var project = _resolveProject(workspaceId, request.TestProjectName);
        _validateIsTestProject(project);
        ...
        return new BatchScaffoldContext(
            ...
            Framework: _resolveTestFramework(request.TestFramework, project.FilePath),
    ...
    /// <summary>
    /// Non-async variant of <c>ResolveTargetTypeAndMethodAsync</c> reading from a cached
    ...
    /// </summary>
    public async Task<RefactoringPreviewDto> PreviewFirstTestFileAsync(
        string workspaceId, ScaffoldFirstTestFileDto request, CancellationToken ct)
    {
        ...
        _validateIsTestProject(testProject);
        ...
        var framework = _resolveTestFramework(request.TestFramework, testProject.FilePath);
    ...
    private ProjectStatusDto ResolveDestinationTestProject(string workspaceId, string? requestedName, Project sourceProject)
    {
        if (!string.IsNullOrWhiteSpace(requestedName))
            return _resolveProject(workspaceId, requestedName);
