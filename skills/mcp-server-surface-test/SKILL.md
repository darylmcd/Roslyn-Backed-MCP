...
1. Resolve and pin the Roslyn prefix per *Tool prefix* above: call each tool whose name **ends in** `server_info` until one returns a Roslyn-shaped response (`connection.state`, `catalogVersion`, `surface.*`). That response must include `connection.state: "ready"`.
2. If **no** candidate returns a Roslyn-shaped response, or the pinned server reports `connection.state` as anything other than `ready` (`initializing`, `degraded`), **stop and report**:

   > *"This skill requires the Roslyn MCP server — some tool whose name ends in `server_info` must be callable, return a Roslyn-shaped response (`connection.state`, `catalogVersion`, `surface.*`), and report `connection.state: ready`. The prefix does not matter: your client assigns it from the registration path, so it may be `mcp__roslyn__…`, `mcp__plugin_roslyn-mcp_roslyn__…`, or anything else. Start the server — for example `dotnet tool run roslynmcp` or ensure the plugin's stdio entry is active in your client config — confirm the Roslyn `server_info` tool returns `ready` under whichever prefix your tool surface exposes, then re-invoke this skill."*
...
The audit needs a C# workspace to load via the `workspace_load` tool (call it under the prefix pinned in Step 1 — see *Tool prefix*). By default the audit targets the **current session's repo root**. Override the target in two equivalent ways:
...
- **Server-required.** No generic non-MCP fallback exists. If **no** tool whose name ends in `server_info` returns a Roslyn-shaped response, or the pinned server's `connection.state` is not `ready`, halt. Resolve once by suffix and then pin the prefix (see *Tool prefix* in Step 1): a missing `mcp__roslyn__`-prefixed literal is **not** grounds to halt, and neither is a missing `mcp__plugin_roslyn-mcp_roslyn__` one — and a non-Roslyn-shaped response from a same-named tool on another server means *try the next candidate*, not halt.
