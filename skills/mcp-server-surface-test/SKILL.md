...
1. Scan your current tool surface for any tool whose name **ends in** `server_info` and call it. The response must include `connection.state: "ready"`.
2. If **no** tool ends in that suffix, the call fails, or `connection.state` is `initializing` / `degraded` / absent, **stop and report**:

   > *"This skill requires the Roslyn MCP server — a tool whose name ends in `server_info` must be callable, and `connection.state` must be `ready`. The prefix does not matter: your client assigns it from the registration path, so it may be `mcp__roslyn__…`, `mcp__plugin_roslyn-mcp_roslyn__…`, or anything else. Start the server — for example `dotnet tool run roslynmcp` or ensure the plugin's stdio entry is active in your client config — confirm the `server_info` tool returns `ready` under whichever prefix your tool surface exposes, then re-invoke this skill."*
...
The audit needs a C# workspace to load via the `workspace_load` tool (resolve it by suffix — see *Tool prefix* in Step 1). By default the audit targets the **current session's repo root**. Override the target in two equivalent ways:
...
- **Server-required.** No generic non-MCP fallback exists. If **no** tool whose name ends in `server_info` is callable, or `connection.state` is not `ready`, halt. Resolve by suffix, never by prefix (see *Tool prefix* in Step 1): a missing `mcp__roslyn__`-prefixed literal is **not** grounds to halt, and neither is a missing `mcp__plugin_roslyn-mcp_roslyn__` one — scan the whole tool surface for the suffix before deciding.
