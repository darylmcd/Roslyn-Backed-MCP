...
> **This prompt is a null-op without the Roslyn MCP server.** If **no** tool whose name ends in `server_info` returns a Roslyn-shaped response in your current tool list, stop and ask the user to start the server. The very first step of Phase -1 verifies this as a hard gate.
...
8. **Operator diagnostic access.** RoslynMcp does not publish operational logs through `notifications/message`. Record whether operator-controlled stderr is available to the run (`yes` / `no`); if unavailable, rely on public correlation identifiers and do not label that client limitation as a protocol defect.
