...
> **This prompt is a null-op without the Roslyn MCP server.** If **no** tool whose name ends in `server_info` returns a Roslyn-shaped response in your current tool list, stop and ask the user to start the server.
