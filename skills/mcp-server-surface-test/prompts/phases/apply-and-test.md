...
11. `validate_workspace(changedFilePaths=null, runTests=false)` — verify `overallStatus` is one of the values in the canonical verdict table at `<Roslyn-Backed-MCP-root>/ai_docs/domains/tool-usage-guide.md#overallstatus-verdict-table`; an `overallStatus` outside that set (or a documented value the tool can never emit) is a FLAG. Probe `changedFilePaths=null` to confirm auto-scoping off `IChangeTracker`. Probe `runTests=true` on the disposable worktree. **Negative probe:** fabricated `changedFilePaths` entry → clean "no related tests" result (not a crash).
