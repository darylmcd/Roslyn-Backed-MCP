...
3a. **Run-end primary-checkout clean check (HARD GATE).** Run `git -C <audited-repo-root> status --porcelain` against the audited repo's **primary checkout** (not the disposable worktree). Compare against the *Isolation baseline* captured at Phase 0 run start. Any new `M` / `A` / `D` / `??` entry that wasn't in the baseline is an **audit-prompt leak** — file as a P1 finding in *MCP server issues* with category `audit-prompt-leak`, citing the offending phase (most leaks come from a mutation phase that forgot to target the disposable worktree). **Exemption — the two canonical run artifacts.** Entries limited to `audit-reports/<timestamp>_<repo-id>_mcp-server-surface-test.md` and `audit-reports/_latest-promotion-scorecard.json` are the run's own expected output, not a leak: if the audited repo *tracks* either path they surface as `M`, and if it ignores them they do not surface at all. Do not file `audit-prompt-leak` for those two paths, and do not revert them — the report and the scorecard are the deliverables. **Exemption — the resume checkpoint.** `<audited-repo-root>/.audit-state.json`, the per-phase checkpoint the portability contract requires, is likewise not a leak: it is an intentional write to the primary checkout and audited repos do not ignore it, so it surfaces as `??`. Do not file `audit-prompt-leak` for it either. Instead, once the run reaches **final closure** — every phase accounted for AND both deliverables written to the primary checkout — DELETE `<audited-repo-root>/.audit-state.json`, then re-run the `git status --porcelain` comparison so the gate evaluates a genuinely clean tree. A run that ends early (context exhaustion, blocked phase, operator abort) leaves the checkpoint in place on purpose so the next session can resume from `nextPhase`; the deletion is gated on closure, not universal. Every *other* new entry is still a P1 leak. The Phase 6 teardown's local check covers Phase 6; this run-end gate is the catch-all for every other phase. **Do not** auto-revert leaked files — the leak is the evidence; report it and let the operator clean up manually after reviewing.
...
The scorecard lives **next to its source evidence** in the audited repo's own `audit-reports/` folder, alongside the prose audit report. As above, `<audited-repo-root>` is the audited repo's **PRIMARY checkout — never the disposable Phase-6 worktree**; if the scorecard was computed while the disposable worktree was current, copy it into the primary checkout's `audit-reports/` **before** worktree teardown. This file is **overwritten** each run. Schema:
