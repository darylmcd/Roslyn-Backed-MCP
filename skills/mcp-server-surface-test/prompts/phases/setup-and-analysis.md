...
4. **Sanity-check the catalog resource.** Read `roslyn://server/catalog` using the live resource server handle selected from `server_info.resourceServerNames` when your client requires one, then confirm per-category counts match `server_info.surface`. A mismatch here is a P2 finding.
...
3. **Operator diagnostic access.** Record whether operator-controlled RoslynMcp stderr is available (`yes` / `no`). Do not expect `notifications/message` protocol logs.
4. Read `roslyn://server/resource-templates` using the same live resource server handle chosen from `server_info.resourceServerNames` to capture all resource URI templates.
