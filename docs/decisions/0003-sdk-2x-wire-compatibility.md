...
| Tasks for slow operations | Additive, opt-in extension | **Decision delivered:** [ADR 0007](0007-tasks-extension-compatibility.md). **Tracked:** `tasks-extension-workspace-load`, `tasks-extension-build-test-run`, and `tasks-extension-contract-docs`. | No migration until enabled. Adoption uses the exact 2.2.0 extension/core pair, protocol 2026-07-28 plus per-request opt-in, and a host-owned allowlist; existing synchronous calls remain valid. |
