...
2. **The blast radius is large.** The backlog item's Evidence records direct removal spanning at least 12 production files and 6 test files. Producers alone span `src/RoslynMcp.Roslyn/Helpers/SymbolMapper.cs`, `src/RoslynMcp.Roslyn/Services/DiagnosticService.cs`, `src/RoslynMcp.Roslyn/Services/CompileCheckService.cs`, `src/RoslynMcp.Roslyn/Services/MutationAnalysisService.cs`, and `src/RoslynMcp.Roslyn/Helpers/DotnetOutputParser.cs`.
...
- `Location` is `null` when the producer could not resolve a span at all. This mirrors the existing flat-field pattern on `SymbolDto` and `DiagnosticDto`, where all five coordinates are nullable and are left null together.
- `Location` is never partially populated. `LocationDto`'s coordinate members are non-nullable value types, so a `LocationDto` instance always carries a full span. A producer holding only a file path and no line/column information emits `Location = null` and populates only the flat `FilePath`, rather than fabricating zero coordinates.
...
- Populate it in the five confirmed producers: `src/RoslynMcp.Roslyn/Helpers/SymbolMapper.cs`, `src/RoslynMcp.Roslyn/Services/DiagnosticService.cs`, `src/RoslynMcp.Roslyn/Services/CompileCheckService.cs`, `src/RoslynMcp.Roslyn/Services/MutationAnalysisService.cs`, `src/RoslynMcp.Roslyn/Helpers/DotnetOutputParser.cs`.
