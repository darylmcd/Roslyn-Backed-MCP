...
GitHub explicitly warns against using self-hosted runners with public repos because **anyone who opens a PR can run arbitrary code on the runner machine** via malicious workflow YAML or build scripts. The mitigation in this repo (**active since 2026-08-10** — the authoritative predicate now lives in the `route` job's maintainer-PR check, `.github/workflows/ci.yml:74`; see [Hosted fallback for an offline runner](#hosted-fallback-for-an-offline-runner)):
...
**Active as of 2026-05-08 (routing mechanism updated 2026-08-10, runner matrix + OS-parity leg added 2026-08-19 — see [Hosted fallback for an offline runner](#hosted-fallback-for-an-offline-runner)).** The CI workflow (`.github/workflows/ci.yml`) splits routing across jobs instead of one inline `runs-on:` conditional: a `route` job (`ci.yml:40-135`) computes the target runner **matrix**, `validate` (`ci.yml:137-321`) fans out over it, and a `validate-gate` aggregator (`ci.yml:330-344`) provides the stable required-check name for branch protection:

```yaml
jobs:
  route:
    outputs:
      runs_on: ${{ steps.decide.outputs.runs_on }}            # legacy single target (kept)
      runner_matrix: ${{ steps.decide.outputs.runner_matrix }} # list of validate legs
    steps:
      - id: decide
        run: |
          $isMaintainerPr = "${{ github.event_name == 'pull_request' && !github.event.pull_request.head.repo.fork && github.event.pull_request.user.login != 'dependabot[bot]' }}"
  ...
  validate-gate:
    ...
    # fails unless every validate leg succeeded — pin THIS in branch protection
```
...
**Rollback if anything breaks:** remove `validate`'s `strategy:` block and set its `runs-on:` (`ci.yml:166`) directly to `ubuntu-latest`, then delete the two `matrix.leg.primary == true` clauses from the upload/coverage step conditions and the `${{ matrix.leg.os }}` job name (and drop `needs: route`, or leave it — an unused `needs` is harmless). Keep the `validate-gate` job (it is the branch-protection required check; with a single validate job it simply mirrors its result), or remove it AND unpin it from branch protection together. The `route` job can stay in place unused, or be deleted along with it. Self-hosted runner stays registered but idle.
...
**Workflow stuck pending after activation.** GitHub queues self-hosted jobs that don't match available runners. Re-verify the label list the `route` job emits (`["self-hosted","roslynmcp-dev"]` inside `$selfHostedMatrix` in `ci.yml`, consumed by `validate`'s `runs-on: ${{ matrix.leg.runs_on }}`) matches the runner's registered labels (`roslynmcp-dev`).

## Removing the runner

If you decide to abandon the self-hosted approach:

1. Revert `.github/workflows/ci.yml`'s `validate` job to a plain `runs-on: ubuntu-latest` (drop the `strategy:` matrix, the `matrix.leg.*` references, and the `route` job entirely, since nothing else depends on them; keep `validate-gate` or re-pin branch protection to the new single check).
