...
- **Backlog:** 8 rows closed under the 2026-04-16 sweep: `dr-9-13-flag-resource-invalid-range-resource-returns-ge` (P4), `dr-9-7-bug-json-parse-surfaces-stack-trace` (P4), `dr-9-9-response-claims` (P4), `project-graph-missing-metadata-fields` (P4), `dr-9-1-regression-r17a-emits-literal-placeholder` (P4), `dr-9-8-bug-validate-fabricated-accepts-fabricated-silen` (P4), `dr-9-11-payload-exceeds-mcp-tool-result-cap` (P4), `change-signature-parameter-span-hint-for-remove` (P4).
- **Tests:** +20 regression tests — 3 in `Top10V2RegressionTests`, 3 in `PromptSmokeTests`, 1 in `WorkspaceExecutionGateTests`, 1 in `WorkspaceToolsIntegrationTests`, 3 in a new `RestructureServiceTests` file, 3 in `ValidateWorkspaceSummaryTests`, 4 in `SurfaceCatalogTests`, and 2 in `ChangeSignaturePreviewTests` (caret-on-parameter auto-resolve). The two pre-existing `*_Throws` source-file-lines tests were rewritten as `*_ReturnsStructuredError` to reflect the new contract.
