...
- **Backlog:** 2 rows closed under the 2026-04-16 sweep: `dr-9-13-flag-resource-invalid-range-resource-returns-ge` (P4), `dr-9-7-bug-json-parse-surfaces-stack-trace` (P4).
- **Tests:** +6 regression tests — 3 in `Top10V2RegressionTests` (end-before-start, non-numeric range, start-past-EOF) and 3 in `PromptSmokeTests` (malformed JSON, non-object JSON, per-parameter type mismatch). The two pre-existing `*_Throws` tests were rewritten as `*_ReturnsStructuredError` to reflect the new contract.
