...
- **Backlog:** 21 rows closed across the v6 top-10 pass and the 2026-04-16 backlog sweep:
  ...
  - **Backlog sweep 2026-04-16 — 8 rows so far:**
    - **P3 (5):** `format-range-preview-empty-diff-compile-check-filter-false-clean`, `change-signature-preview-brace-concat-on-add-op`, `dr-9-7-reports-during-refactoring-transitions-but-the-w`, `find-references-preview-text-inflates-response`, `change-signature-preview-callsite-summary`.
...
- **Tests:** 497 → 534 (+37 across 13 new regression test files; +1 added inline to `ChangeSignaturePreviewTests`). Each item ships with a targeted regression suite covering the specific audit repro. Zero test regressions; zero build warnings.
