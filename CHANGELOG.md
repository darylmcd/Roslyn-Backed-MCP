...
- **Backlog:** 21 rows closed across the v6 top-10 pass and the 2026-04-16 backlog sweep:
  ...
  - **Backlog sweep 2026-04-16 — 6 rows so far:**
    - **P3 (3):** `format-range-preview-empty-diff-compile-check-filter-false-clean`, `change-signature-preview-brace-concat-on-add-op`, `dr-9-7-reports-during-refactoring-transitions-but-the-w`.
...
- **Tests:** 497 → 530 (+33 across 12 new regression test files; +3 added inline to `WorkspaceStatusSummaryDtoTests`). Each item ships with a targeted regression suite covering the specific audit repro. Zero test regressions; zero build warnings.
