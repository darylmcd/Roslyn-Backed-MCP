...
- **Backlog:** 21 rows closed across the v6 top-10 pass and the 2026-04-16 backlog sweep:
  ...
  - **Backlog sweep 2026-04-16 — 3 rows so far:**
    - **P3 (2):** `format-range-preview-empty-diff-compile-check-filter-false-clean`, `change-signature-preview-brace-concat-on-add-op`.
    - **P4 (1):** `dr-9-12-flag-format-range-empty-returns-empty-diff-on-d`.
- **Tests:** 497 → 527 (+30 across 12 new regression test files). Each item ships with a targeted regression suite covering the specific audit repro. Zero test regressions; zero build warnings.
