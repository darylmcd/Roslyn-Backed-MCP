...
- **Backlog:** 21 rows closed across the v6 top-10 pass and the 2026-04-16 backlog sweep:
  ...
  - **Backlog sweep 2026-04-16 — 12 rows so far:**
    - **P3 (9):** `format-range-preview-empty-diff-compile-check-filter-false-clean`, `change-signature-preview-brace-concat-on-add-op`, `dr-9-7-reports-during-refactoring-transitions-but-the-w`, `find-references-preview-text-inflates-response`, `change-signature-preview-callsite-summary`, `get-nuget-dependencies-no-summary-mode`, `get-syntax-tree-max-output-chars-incomplete-cap`, `symbol-impact-sweep-output-size-blowup`, `validate-workspace-output-cap-summary-mode`.
...
- **Tests:** 497 → 546 (+49 across 17 new regression test files; +1 added inline to `ChangeSignaturePreviewTests`). Each item ships with a targeted regression suite covering the specific audit repro. Zero test regressions; zero build warnings.
