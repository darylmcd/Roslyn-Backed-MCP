...
- **Backlog:** 19 rows closed in the final sync commit:
...
- **Tests:** 497 → 522 (+25 across 10 new regression test files). Each item ships with a targeted regression suite covering the specific audit repro. Zero test regressions; zero build warnings.
- **Plan / audit intake:** New file `ai_docs/plans/20260415T220000Z_top10-remediation-plan.md` (the source-code-verified plan drafted before the implementation pass). New audit file `ai_docs/audit-reports/20260415T193603Z_jellyfin_stress-test.md`. Both linked from `ai_docs/backlog.md` Refs.
