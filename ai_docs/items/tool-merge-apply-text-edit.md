# tool-merge-apply-text-edit — Merge 3 apply tools into `text_edit_apply`

**row:** `tool-merge-apply-text-edit` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `apply_text_edit`, `apply_multi_file_edit`, `preview_multi_file_edit_apply` are served by one kind-discriminated `text_edit_apply`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `apply_text_edit` — `src/RoslynMcp.Host.Stdio/Tools/EditTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
- `apply_multi_file_edit` — `src/RoslynMcp.Host.Stdio/Tools/MultiFileEditTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
- `preview_multi_file_edit_apply` — `src/RoslynMcp.Host.Stdio/Tools/MultiFileEditTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
...
Risk bucket: **text-edit** (3 applies).
