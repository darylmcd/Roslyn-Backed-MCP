# promotion-tier-analysis-batch-2 — Promote qualifying experimental tools in the Analysis catalog partial (batch 2 of 3)

**row:** `promotion-tier-analysis-batch-2` · **pri:** `Medium` · **size:** `M` · **deps:** `promotion-scorecard-refresh-toplevel-run`

## Anchors

- `src/RoslynMcp.Host.Stdio/Tools/ImpactSweepTools`
- `src/RoslynMcp.Host.Stdio/Tools/RecordImpactTools`
- `src/RoslynMcp.Host.Stdio/Tools/TestReferenceMapTools`
- `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Analysis.cs`

## Acceptance

- [ ] Each tool in this batch that the REFRESHED scorecard qualifies is promoted experimental -> stable: the `[McpToolMetadata]` tier argument and the matching `ServerSurfaceCatalog.Analysis.cs` `Tool(...)` row change together.
...
- `ImpactSweepTools` — `symbol_impact_sweep`
- `RecordImpactTools` — `preview_record_field_addition`
- `TestReferenceMapTools` — `test_reference_map`

Batch candidates: **3**. Whole surface at split time: 174 tools = 113 stable + 61 experimental.
