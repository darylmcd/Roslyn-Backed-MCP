# param-dedupe-cohesion-orchestration-consumer — parameter-description dedupe: Cohesion, orchestration and consumer slice

**row:** `param-dedupe-cohesion-orchestration-consumer` · **pri:** `Low` · **size:** `M` · **deps:** `desc-budget-harness-param-family`
...
- `tests/RoslynMcp.Tests/ParamDescriptionCanonicalCohesionOrchestrationConsumerTests.cs` (new — calls the shared harness)
...
Measured on `main` 2026-09-02: this slice carries **47 parameter `[Description]`s totalling 3023 chars** across 4 file(s). Whole-surface uncovered remainder at split time: 36 files, 528 params, 41,760 chars; 16 files (186 params, 12,774 chars) are already covered by the two shipped canonicalization ratchets (PRs #1349, #1350).
