# param-dedupe-workspace-validation-scaffolding — parameter-description dedupe: Workspace, validation and scaffolding slice

**row:** `param-dedupe-workspace-validation-scaffolding` · **pri:** `Low` · **size:** `M` · **deps:** `desc-budget-harness-param-family`
...
- `tests/RoslynMcp.Tests/ParamDescriptionCanonicalWorkspaceValidationScaffoldingTests.cs` (new — calls the shared harness)
...
Measured on `main` 2026-09-02: this slice carries **108 parameter `[Description]`s totalling 9487 chars** across 4 file(s). Whole-surface uncovered remainder at split time: 36 files, 528 params, 41,760 chars; 16 files (186 params, 12,774 chars) are already covered by the two shipped canonicalization ratchets (PRs #1349, #1350).
