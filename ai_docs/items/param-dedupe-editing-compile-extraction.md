# param-dedupe-editing-compile-extraction — parameter-description dedupe: Editing, compile-check and extraction slice

**row:** `param-dedupe-editing-compile-extraction` · **pri:** `Low` · **size:** `M` · **deps:** `desc-budget-harness-param-family`
...
- `tests/RoslynMcp.Tests/ParamDescriptionCanonicalEditingCompileExtractionTests.cs` (new — calls the shared harness)
...
Measured on `main` 2026-09-02: this slice carries **42 parameter `[Description]`s totalling 4091 chars** across 4 file(s). Whole-surface uncovered remainder at split time: 36 files, 528 params, 41,760 chars; 16 files (186 params, 12,774 chars) are already covered by the two shipped canonicalization ratchets (PRs #1349, #1350).
