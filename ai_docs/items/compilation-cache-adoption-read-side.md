...
**Batch b (group-b preview-path partial slice) SHIPPED:** [#1147](https://github.com/darylmcd/Roslyn-Backed-MCP/pull/1147) — `BuildService`, `FixAllService`, `BulkRefactoringService`, `CrossProjectRefactoringService`, 4 of group (b)'s listed sites. Remaining group-b tail: `InterfaceExtractionService:494`, `ScaffoldingService.*`. Row deliberately left OPEN (`backlogRowsClosed: []`) — this was a bounded child batch of a size-L row, not the full close.

**New hazard found during PR #1147 code-quality review (amend, not a new row — do not file a sibling):** `CompilationCache.GetCompilationAsync` (`CompilationCache.cs:56-80`) caches the `Task` created with the FIRST caller's `CancellationToken` and returns it on later hits with only a version check — no task-status check. A canceled/faulted task stays installed until the workspace version bumps, so an unrelated caller at the same version observes `OperationCanceledException` from a token it never canceled. PR #1147 extended this exposure to 4 more services including user-facing fix-all and bulk/cross-project preview paths. Fix: detach cached tasks from any single caller's token (start with `CancellationToken.None`, let callers observe their own `ct` separately) and evict entries whose task completed canceled/faulted instead of serving them. Regression test: caller A cancels mid-fetch, caller B requests the same `(workspaceId, project)` at the unchanged version and receives a real `Compilation` instead of `OperationCanceledException`.
