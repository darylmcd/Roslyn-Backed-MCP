# tool-merge-preview-split-class — Merge 1 preview tool into `restructure_preview`

**row:** `tool-merge-preview-split-class` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery,tool-merge-preview-restructure`
...
- [ ] `split_class_preview` are served by one kind-discriminated `restructure_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `split_class_preview` — `src/RoslynMcp.Host.Stdio/Tools/OrchestrationTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Orchestration.cs`
...
Wave 2 of the restructure family — absorbs `split_class_preview` into the `restructure_preview` established by `tool-merge-preview-restructure`.
