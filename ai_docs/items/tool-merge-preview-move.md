# tool-merge-preview-move — Merge 2 preview tools into `move_preview`

**row:** `tool-merge-preview-move` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `move_type_to_file_preview`, `rename_preview` are served by one kind-discriminated `move_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `move_type_to_file_preview` — `src/RoslynMcp.Host.Stdio/Tools/TypeMoveTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `rename_preview` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
...
**Judgment flag for the ADR:** `rename_preview` is a stable, heavily-used tool; folding it into a `move_preview` kind may not be worth the churn. If the ADR rejects this pairing, close this row rather than forcing the merge.
