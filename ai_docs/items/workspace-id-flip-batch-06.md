# workspace-id-flip-batch-06 — flip read-only `workspaceId` to optional (batch 6 of 11)

**row:** `workspace-id-flip-batch-06` · **pri:** `Low` · **size:** `M` · **deps:** `workspace-id-optional-adoption-evidence,workspace-id-optional-named-argument-prerequisite`
...
- `tests/RoslynMcp.Tests/WorkspaceIdOptionalBatch06Tests.cs` (new — per-batch reflection assertion)
...
|---|---|
| `TypeMoveTools.cs` | 2 |
| `ValidationBundleTools.cs` | 2 |
| `AnalyzerInfoTools.cs` | 1 |
| `ChangeSignatureTools.cs` | 1 |

Batch total: **6 tools across 4 file(s)**. Whole remainder at split time: **121 tools across 43 files** — materially more than the retired parent row's "~45 read-only tools" estimate.
