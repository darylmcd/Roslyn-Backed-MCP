# param-dedupe-verify-suggestion-snippet-drift — parameter-description dedupe: Apply-with-verify, suggestion and snippet slice

**row:** `param-dedupe-verify-suggestion-snippet-drift` · **pri:** `Low` · **size:** `M` · **deps:** `desc-budget-harness-param-family`

## Anchors

- `src/RoslynMcp.Host.Stdio/Tools/ApplyWithVerifyTool.cs`
- `src/RoslynMcp.Host.Stdio/Tools/SuggestionTools.cs`
- `src/RoslynMcp.Host.Stdio/Tools/SnippetAnalysisTools.cs`
- `src/RoslynMcp.Host.Stdio/Tools/WorkspaceDriftTool.cs`
- `tests/RoslynMcp.Tests/ParamDescriptionCanonicalVerifySuggestionSnippetDriftTests.cs` (new — calls the shared harness)
...
Measured on `main` 2026-09-02: this slice carries **8 parameter `[Description]`s totalling 459 chars** across 4 file(s). Whole-surface uncovered remainder at split time: 36 files, 528 params, 41,760 chars; 16 files (186 params, 12,774 chars) are already covered by the two shipped canonicalization ratchets (PRs #1349, #1350).
