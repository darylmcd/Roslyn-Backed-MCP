...
## Evidence update — 2026-08-29 (PR #1399)

Hosted shard `windows-hosted-3-of-4` failed `ExecuteAsync_HardDeadlinePreservesOutcomeAndAggregatesCleanupFailures` at its 10-second MSTest timeout while the local authoritative suite passed. Re-derivation found the test-only `FailingCleanupWorkerProcess.Session.WaitForExit(milliseconds)` returned `false` immediately instead of honoring the interface's bounded-wait contract, busy-spinning until the one-second hard deadline and becoming scheduler-starved under shard contention. The fixture now sleeps for the requested interval before returning `false`, preserving the never-exits behavior without the CPU spin. This advances the ScriptingService timing-flake scope; the row remains open for its other acceptance bullets.
