# workspace-id-flip-batch-03 — flip read-only `workspaceId` to optional (batch 3 of 11)

**row:** `workspace-id-flip-batch-03` · **pri:** `Low` · **size:** `M` · **deps:** `workspace-id-optional-adoption-evidence,workspace-id-optional-named-argument-prerequisite`

## Anchors

- `src/RoslynMcp.Host.Stdio/Tools/FileOperationTools.cs`
- `src/RoslynMcp.Host.Stdio/Tools/MSBuildTools.cs`
- `src/RoslynMcp.Host.Stdio/Tools/OrchestrationTools.cs`
- `src/RoslynMcp.Host.Stdio/Tools/SecurityTools.cs`
- `tests/RoslynMcp.Tests/WorkspaceIdOptionalBatch03Tests.cs` (new — per-batch reflection assertion)
...
|---|---|
| `FileOperationTools.cs` | 3 |
| `MSBuildTools.cs` | 3 |
| `OrchestrationTools.cs` | 3 |
| `SecurityTools.cs` | 3 |

Batch total: **12 tools across 4 file(s)**. Whole remainder at split time: **121 tools across 43 files** — materially more than the retired parent row's "~45 read-only tools" estimate.
