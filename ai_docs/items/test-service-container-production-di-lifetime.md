...
Plan 20260910T192234Z_backlog-remediate bundles workspace-manager-file-watcher-disposal-ownership: AddRoslynServices provider disposal and WorkspaceManager.Dispose are the same causal ownership path, and one exactly-once watcher regression proves both.
Quality correction (2026-09-11): TestServiceContainer now exposes the provider-resolved DuplicateMethodDetectorService through TestBase, and AliasToolsTests no longer constructs a local Lazy instance. The existing canonical-plus-alias parity test exercises the production registration for both routes.
