...
- `tests/RoslynMcp.Tests/WorkspaceIdOptionalBatch11Tests.cs` (new — per-batch reflection assertion)

## Acceptance

- [ ] Every read-only tool in these 3 file(s) takes `workspaceId` as OPTIONAL, with a `[Description]` update inviting omission and the residual-case `RequireResolvedWorkspaceId` guard — mirroring the `SymbolTools.cs` pilot pattern.
...
|---|---|
| `TestReferenceMapTools.cs` | 1 |
| `TypeExtractionTools.cs` | 1 |
| `WorkspaceDriftTool.cs` | 1 |

Batch total: **3 tools across 3 file(s)**. Whole remainder at split time: **121 tools across 43 files** — materially more than the retired parent row's "~45 read-only tools" estimate.
