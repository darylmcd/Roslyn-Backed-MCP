# tool-merge-preview-formatting — Merge 3 preview tools into `format_preview`

**row:** `tool-merge-preview-formatting` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `format_document_preview`, `format_range_preview`, `organize_usings_preview` are served by one kind-discriminated `format_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `format_document_preview` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `format_range_preview` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `organize_usings_preview` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
