# tool-merge-apply-code-transform-2 — Merge 3 apply tools into `code_transform_apply`

**row:** `tool-merge-apply-code-transform-2` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery,tool-merge-apply-code-transform-1`
...
- [ ] `extract_interface_apply`, `extract_method_apply`, `extract_type_apply` are served by one kind-discriminated `code_transform_apply`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `extract_interface_apply` — `src/RoslynMcp.Host.Stdio/Tools/InterfaceExtractionTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `extract_method_apply` — `src/RoslynMcp.Host.Stdio/Tools/ExtractMethodTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `extract_type_apply` — `src/RoslynMcp.Host.Stdio/Tools/TypeExtractionTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
...
Risk bucket: **code-transform**, wave 2 — absorbs the extract kinds.
