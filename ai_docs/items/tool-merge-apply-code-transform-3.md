# tool-merge-apply-code-transform-3 — Merge 3 apply tools into `code_transform_apply`

**row:** `tool-merge-apply-code-transform-3` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery,tool-merge-apply-code-transform-2`
...
- [ ] `move_type_to_file_apply`, `bulk_replace_type_apply`, `rename_apply` are served by one kind-discriminated `code_transform_apply`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `move_type_to_file_apply` — `src/RoslynMcp.Host.Stdio/Tools/TypeMoveTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `bulk_replace_type_apply` — `src/RoslynMcp.Host.Stdio/Tools/BulkRefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `rename_apply` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
...
Risk bucket: **code-transform**, wave 3. `rename_apply` is stable and heavily used — confirm against the ADR before aliasing it.
