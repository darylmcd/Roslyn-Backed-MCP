# tool-merge-preview-cross-project — Merge 3 preview tools into `cross_project_preview`

**row:** `tool-merge-preview-cross-project` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`

## Anchors

- `src/RoslynMcp.Host.Stdio/Tools/CrossProjectRefactoringTools.cs`
- `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Orchestration.cs`

## Acceptance

- [ ] `move_type_to_project_preview`, `extract_interface_cross_project_preview`, `dependency_inversion_preview` are served by one kind-discriminated `cross_project_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `move_type_to_project_preview` — `src/RoslynMcp.Host.Stdio/Tools/CrossProjectRefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Orchestration.cs`
- `extract_interface_cross_project_preview` — `src/RoslynMcp.Host.Stdio/Tools/CrossProjectRefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Orchestration.cs`
- `dependency_inversion_preview` — `src/RoslynMcp.Host.Stdio/Tools/CrossProjectRefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Orchestration.cs`
