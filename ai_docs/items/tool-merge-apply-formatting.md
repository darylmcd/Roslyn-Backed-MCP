# tool-merge-apply-formatting — Merge 3 apply tools into `format_apply`

**row:** `tool-merge-apply-formatting` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `format_document_apply`, `format_range_apply`, `organize_usings_apply` are served by one kind-discriminated `format_apply`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `format_document_apply` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `format_range_apply` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `organize_usings_apply` — `src/RoslynMcp.Host.Stdio/Tools/RefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
...
Risk bucket: **formatting** (3 applies).
