# tool-merge-preview-remove — Merge 2 preview tools into `remove_preview`

**row:** `tool-merge-preview-remove` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `remove_dead_code_preview`, `remove_interface_member_preview` are served by one kind-discriminated `remove_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `remove_dead_code_preview` — `src/RoslynMcp.Host.Stdio/Tools/DeadCodeTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
- `remove_interface_member_preview` — `src/RoslynMcp.Host.Stdio/Tools/RemoveInterfaceMemberTool.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
