# tool-merge-preview-extract — Merge 4 preview tools into `extract_preview`

**row:** `tool-merge-preview-extract` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `extract_method_preview`, `extract_type_preview`, `extract_interface_preview`, `extract_shared_expression_to_helper_preview` are served by one kind-discriminated `extract_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `extract_method_preview` — `src/RoslynMcp.Host.Stdio/Tools/ExtractMethodTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `extract_type_preview` — `src/RoslynMcp.Host.Stdio/Tools/TypeExtractionTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `extract_interface_preview` — `src/RoslynMcp.Host.Stdio/Tools/InterfaceExtractionTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `extract_shared_expression_to_helper_preview` — `src/RoslynMcp.Host.Stdio/Tools/ExtractMethodTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
