# tool-merge-preview-replace — Merge 4 preview tools into `replace_preview`

**row:** `tool-merge-preview-replace` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `bulk_replace_type_preview`, `replace_invocation_preview`, `replace_string_literals_preview`, `change_type_namespace_preview` are served by one kind-discriminated `replace_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `bulk_replace_type_preview` — `src/RoslynMcp.Host.Stdio/Tools/BulkRefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `replace_invocation_preview` — `src/RoslynMcp.Host.Stdio/Tools/BulkRefactoringTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `replace_string_literals_preview` — `src/RoslynMcp.Host.Stdio/Tools/RestructureTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
- `change_type_namespace_preview` — `src/RoslynMcp.Host.Stdio/Tools/TypeMoveTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`
