# tool-merge-preview-file-operations — Merge 3 preview tools into `file_operation_preview`

**row:** `tool-merge-preview-file-operations` · **pri:** `Medium` · **size:** `M` · **deps:** `tool-consolidation-adr-and-alias-machinery`
...
- [ ] `create_file_preview`, `delete_file_preview`, `move_file_preview` are served by one kind-discriminated `file_operation_preview`, mirroring the pattern `symbol_refactor_preview` already ships.
...
- `create_file_preview` — `src/RoslynMcp.Host.Stdio/Tools/FileOperationTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
- `delete_file_preview` — `src/RoslynMcp.Host.Stdio/Tools/FileOperationTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
- `move_file_preview` — `src/RoslynMcp.Host.Stdio/Tools/FileOperationTools.cs` / `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Editing.cs`
