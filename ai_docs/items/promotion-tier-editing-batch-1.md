...
- [ ] Each tool in this batch that the REFRESHED scorecard qualifies is promoted experimental -> stable: the `[McpToolMetadata]` tier argument and the matching `ServerSurfaceCatalog.Editing.cs` `Tool(...)` row change together.
...
- `DeadCodeTools` — `remove_dead_code_apply`
- `FileOperationTools` — `create_file_apply`, `delete_file_apply`, `move_file_apply`
- `MultiFileEditTools` — `apply_multi_file_edit`, `preview_multi_file_edit`, `preview_multi_file_edit_apply`

Batch candidates: **7**. Whole surface at split time: 174 tools = 113 stable + 61 experimental.
