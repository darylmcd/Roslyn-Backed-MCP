# workspace-id-flip-batch-02 — flip read-only `workspaceId` to optional (batch 2 of 11)

**row:** `workspace-id-flip-batch-02` · **pri:** `Low` · **size:** `M` · **deps:** `workspace-id-optional-adoption-evidence,workspace-id-optional-named-argument-prerequisite`
...
- `tests/RoslynMcp.Tests/WorkspaceIdOptionalBatch02Tests.cs` (new — per-batch reflection assertion)
...
|---|---|
| `RefactoringTools.cs` | 6 |
| `WorkspaceTools.cs` | 6 |
| `ScaffoldingTools.cs` | 4 |
| `CrossProjectRefactoringTools.cs` | 3 |

Batch total: **19 tools across 4 file(s)**. Whole remainder at split time: **121 tools across 43 files** — materially more than the retired parent row's "~45 read-only tools" estimate.
