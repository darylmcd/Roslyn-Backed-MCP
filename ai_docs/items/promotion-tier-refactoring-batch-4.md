# promotion-tier-refactoring-batch-4 — Promote qualifying experimental tools in the Refactoring catalog partial (batch 4 of 4)

**row:** `promotion-tier-refactoring-batch-4` · **pri:** `Medium` · **size:** `M` · **deps:** `promotion-scorecard-refresh-toplevel-run`

## Anchors

- `src/RoslynMcp.Host.Stdio/Tools/SymbolRefactorTools`
- `src/RoslynMcp.Host.Stdio/Tools/TypeExtractionTools`
- `src/RoslynMcp.Host.Stdio/Tools/TypeMoveTools`
- `src/RoslynMcp.Host.Stdio/Catalog/ServerSurfaceCatalog.Refactoring.cs`

## Acceptance

- [ ] Each tool in this batch that the REFRESHED scorecard qualifies is promoted experimental -> stable: the `[McpToolMetadata]` tier argument and the matching `ServerSurfaceCatalog.Refactoring.cs` `Tool(...)` row change together.
...
- `SymbolRefactorTools` — `symbol_refactor_preview`, `split_service_with_di_preview`, `record_field_add_with_satellites_preview`
- `TypeExtractionTools` — `extract_type_apply`
- `TypeMoveTools` — `move_type_to_file_apply`, `change_type_namespace_preview`

Batch candidates: **6**. Whole surface at split time: 174 tools = 113 stable + 61 experimental.
