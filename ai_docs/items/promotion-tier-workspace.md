...
- [ ] Each tool in this batch that the REFRESHED scorecard qualifies is promoted experimental -> stable: the `[McpToolMetadata]` tier argument and the matching `ServerSurfaceCatalog.Workspace.cs` `Tool(...)` row change together.
...
- `WorkspaceDriftTool` — `workspace_drift_check`
- `WorkspaceWarmTools` — `workspace_warm`

Batch candidates: **2**. Whole surface at split time: 174 tools = 113 stable + 61 experimental.
