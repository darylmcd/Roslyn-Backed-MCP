# workspace-id-flip-batch-04 — flip read-only `workspaceId` to optional (batch 4 of 11)

**row:** `workspace-id-flip-batch-04` · **pri:** `Low` · **size:** `M` · **deps:** `workspace-id-optional-adoption-evidence,workspace-id-optional-named-argument-prerequisite`
...
- `tests/RoslynMcp.Tests/WorkspaceIdOptionalBatch04Tests.cs` (new — per-batch reflection assertion)
...
|---|---|
| `SymbolRefactorTools.cs` | 3 |
| `ValidationTools.cs` | 3 |
| `BulkRefactoringTools.cs` | 2 |
| `CodeActionTools.cs` | 2 |

Batch total: **10 tools across 4 file(s)**. Whole remainder at split time: **121 tools across 43 files** — materially more than the retired parent row's "~45 read-only tools" estimate.
