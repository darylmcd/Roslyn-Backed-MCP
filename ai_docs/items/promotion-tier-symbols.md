# promotion-tier-symbols — Promote qualifying experimental tools in the Symbols catalog partial

**row:** `promotion-tier-symbols` · **pri:** `Medium` · **size:** `M` · **deps:** `promotion-scorecard-refresh-toplevel-run`
...
- [ ] Each tool in this batch that the REFRESHED scorecard qualifies is promoted experimental -> stable: the `[McpToolMetadata]` tier argument and the matching `ServerSurfaceCatalog.Symbols.cs` `Tool(...)` row change together.
...
- `SymbolTools` — `find_overloads`, `find_type_consumers`, `probe_position`
