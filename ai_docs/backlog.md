...
**updated_at:** 2026-05-07T14:05:48Z
...
| `parameter-object-preview-design` | Low | none | Produce a bounded design note for a parameter-object conversion preview, then split an implementation row only if the design fits the backlog size limits. The recurring need is replacing many parameters with one record/DTO while updating callsites; this is broader than `change_signature_preview` add/remove/rename and broader than `replace_invocation_preview`'s method-to-method rewrite. Anchors: `src/RoslynMcp.Roslyn/Services/ChangeSignatureService.cs`, `src/RoslynMcp.Roslyn/Services/BulkRefactoringService.cs`, `src/RoslynMcp.Host.Stdio/Tools/ChangeSignatureTools.cs`, `src/RoslynMcp.Host.Stdio/Tools/BulkRefactoringTools.cs`. Regression test shape: design-only artifact under `ai_docs/items/parameter-object-preview-design.md` with proposed DTO/tool contract, file-count estimate, and go/no-go split recommendation. Evidence: 2026-04-26 multi-session retro sessions `34ca7601`, `d8763f40`, `dd0a7e48`. |
