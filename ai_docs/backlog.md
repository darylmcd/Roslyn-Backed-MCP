...
| `validate-locator-preflight-tool` | Low | `inv-arg-envelope-schema-hint` | Add a read-only `validate_locator` tool for pre-flight locator validation. Input `{ filePath?, line?, column?, symbolHandle?, metadataName? }`, output `{ valid, mode, normalized, hint }`. Lets agents catch malformed `metadataName` / `symbolHandle` before paying the round-trip cost of `find_references` / `symbol_relationships` / etc. Re-measure after `inv-arg-envelope-schema-hint` ships — if a good schema hint already lets agents self-correct, this row may close obsolete. Anchors: new tool surface in `src/RoslynMcp.Host.Stdio/Tools/SymbolLocatorTool.cs`, reuses existing `src/RoslynMcp.Host.Stdio/Tools/SymbolLocatorFactory.cs` helper from PR #474, catalog registration. Regression test shape: 1 fixture covering valid file/line, valid metadataName, malformed metadataName (parenthesized), unparseable symbolHandle, fully empty locator. Evidence: `ai_docs/reports/20260504T200153Z_roslyn-backed-mcp_roslyn-mcp-multisession-retro.md` §2b row 1 / §4#4 — 6 sessions hit the post-hoc shape error and would have benefited from pre-flight validation. |
| `find-references-project-filter` | Low | none | Add optional `projectFilter: string \| string[]` parameter to `find_references` and `find_consumers` tools so callers can scope a reference walk to a project subset without post-hoc filtering. `semantic_grep` already accepts `projectFilter` — the surface inconsistency is itself an ergonomic friction. Most useful on >100-project solutions where the unfiltered result is large. Anchors: `src/RoslynMcp.Host.Stdio/Tools/AnalysisTools.cs` (find_references / find_consumers tool wrappers), `src/RoslynMcp.Roslyn/Services/SymbolReferenceService.cs` and `src/RoslynMcp.Roslyn/Services/ConsumerAnalysisService.cs` (service-layer filter). Regression test shape: 2 tests per tool — unfiltered baseline, filter-to-single-project narrows correctly. Evidence: `ai_docs/reports/20260504T200153Z_roslyn-backed-mcp_roslyn-mcp-multisession-retro.md` §2b row 3 / §4#5 — 3 DotNet-Network-Documentation sessions (200+ project solution) walked results post-hoc. |
