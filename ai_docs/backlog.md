...
**updated_at: 2026-05-08T21:21:43Z**
...
| `scaffold-test-internal-target-accessibility` | Medium | none | `scaffold_test_preview` can generate a test that does not compile when the target type or method is inaccessible from the test project. Repro: scaffold a MSTest method test for internal `RoslynMcp.Core.Audit.AuditScratch.CallBadName`; `scaffold_test_apply` succeeds, but `compile_check` reports `CS0122` for the internal class and method. Next deliverable: have scaffolding inspect target/test assembly accessibility and either emit a warning plus non-applicable preview, suggest `InternalsVisibleTo`, or generate only compile-safe placeholders. Anchors: `src/RoslynMcp.Roslyn/Services/ScaffoldingService.cs:329`, `src/RoslynMcp.Host.Stdio/Tools/ScaffoldingTools.cs:92`. Regression test shape: 1 fixture covering internal-target rejection (or warning-with-placeholder), public-target success, and `InternalsVisibleTo`-already-present success. Evidence: `review-inbox/archive/20260508T154415Z/20260508T154415Z_roslyn-backed-mcp_mcp-server-audit.md` Phase 12 + fragment id `roslyn-backed-mcp-scaffold-test-internal-target-accessibility`. |
