...
**updated_at:** 2026-04-27T21:03:46Z
...
| id | pri | deps | do |
|----|-----|------|-----|
| `change-signature-preview-metadataname-shape-error-actionability` | Medium | none | `change_signature_preview` rejects fully-qualified method names of the form `Namespace.Type.Method(string)` (with parenthesized signature) with the vague error "requires a method symbol; resolved null", forcing agents to rediscover the right locator shape. Either accept the parenthesized form OR add an example to the `metadataName` parameter description showing the supported form (bare method name + position, or `symbolHandle` from `symbol_search`). Anchors: tool registration `src/RoslynMcp.Host.Stdio/Tools/ChangeSignatureTools.cs`, service `src/RoslynMcp.Roslyn/Services/ChangeSignatureService.cs`, locator resolver in `src/RoslynMcp.Core/Models/SymbolLocator.cs`. Regression test shape: pass parenthesized metadataName → expect either success OR a rejection that names the shape mismatch and points at the supported form. Evidence: `review-inbox/archive/20260427T202515Z/20260427T202515Z_roslyn-backed-mcp_mcp-server-audit.md` §7 + §15 S-05 (self-audit 2026-04-27, blocks promotion of `change_signature_preview` from experimental). |
| `mcp-error-category-workspace-evicted-on-host-recycle` | Medium | none | When the MCP stdio host process gracefully recycles between turns (observed: PID 36236 → 52252 with `previousRecycleReason="graceful"`), every workspace-scoped tool call from the prior session returns a bare `KeyNotFoundException` with `category=NotFound` — indistinguishable from a typo'd workspaceId. Add a distinct error category (e.g. `WorkspaceEvicted`) emitted when `serverStartedAt > workspaceLoadedAt`, so agents can auto-reload without ambiguity. Anchors: error envelope construction in `src/RoslynMcp.Host.Stdio/Tools/ToolErrorHandler.cs` (or `StructuredCallToolFilter.cs`), workspace lookup in `src/RoslynMcp.Roslyn/Services/WorkspaceManager.cs`, recycle metadata in `server_heartbeat` response (`src/RoslynMcp.Host.Stdio/Tools/HeartbeatTools.cs` or equivalent). Regression test shape: simulate `workspaceLoadedAt < serverStartedAt` → assert error category=`WorkspaceEvicted` with the recycle timestamps in the envelope. Evidence: `review-inbox/archive/20260427T202515Z/20260427T202515Z_roslyn-backed-mcp_mcp-server-audit.md` §14.3 + §15 S-01 (observed once during self-audit 2026-04-27 between Phase 1 and Phase 2). |
