...
| `cycle-detection-helper-dedup` | Medium | none | `WouldCreateProjectReferenceCycle` is duplicated verbatim (similarity=1.0, 31-41 lines) across `CrossProjectRefactoringService` and `ProjectMutationService`. Promote both to a shared `ProjectGraphHelpers.WouldCreateProjectReferenceCycle(Project source, Project target)` static under `Helpers/`; delete the two private copies. Anchors: `src/RoslynMcp.Roslyn/Services/CrossProjectRefactoringService.cs:748-778`; `src/RoslynMcp.Roslyn/Services/ProjectMutationService.cs:658-688`; new `src/RoslynMcp.Roslyn/Helpers/ProjectGraphHelpers.cs`. Regression test shape: `ProjectMutationIntegrationTests` + `CrossProjectRefactoringService` cycle-rejection tests cover both call sites; add a direct helper unit asserting acyclic and cyclic graphs. Evidence: `mcp__roslyn__find_duplicated_methods` normalizedHash 67e8c5784e6d0a3a / e94477b5aae4d96d. Source: 2026-05-26 discovery-sweep refactor audit. |
