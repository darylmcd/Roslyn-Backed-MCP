...
| `workspace-warm-default-above-50-projects` | Medium | none — requires the shipped `workspace_load` prewarm flag | Default `workspace_load` to run the new prewarm path when the loaded solution exceeds a project-count threshold (suggest 50), while preserving explicit `prewarm: false` opt-out. The prewarm flag is nullable at the tool boundary: omitted currently behaves cold, `true` warms, and `false` is explicit opt-out; change only the omitted branch. Do not pursue persisted compilation/semantic-model warm artifacts here — Roslyn's warm state is process-local. Anchors: `src/RoslynMcp.Host.Stdio/Tools/WorkspaceTools.cs`, `src/RoslynMcp.Roslyn/Services/WorkspaceWarmService.cs`. Regression test shape: 1 above-threshold test (warm fires automatically), 1 below-threshold test (warm doesn't fire), 1 explicit `prewarm: false` opt-out test. Evidence: retro §4 #6; OrchardCore profile cold/warm gap (workspace_load 44.85s vs subsequent symbol_search 1.18s). Source: 2026-05-05 MCP-best-practices comparison §3 rec F. See `ai_docs/reports/20260505_mcp-best-practices-restored-rows.md` for provenance. |
