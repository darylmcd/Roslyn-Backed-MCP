...
| id | pri | deps | do |
|----|-----|------|-----|
| `tool-output-schema-casing-mismatch` | High | none | The `outputSchema` published per tool by `ToolOutputSchemaIndex.GenerateSchema` is generated against `JsonSerializerOptions.Default` (PascalCase property names) while runtime serialization of `structuredContent` uses camelCase via `JsonDefaults.Indented`. Result: clients using the published schema to validate `structuredContent` bodies will see schema-vs-payload casing drift — schema says `Version`, body has `version`. 1-line fix: pass `JsonDefaults.Indented` (or whatever camelCase-configured options instance the `StructuredCallToolFilter` uses for serialization) into `JsonSchemaExporter` instead of `JsonSerializerOptions.Default`. Anchors: `src/RoslynMcp.Host.Stdio/Catalog/ToolOutputSchemaIndex.cs`, `tests/RoslynMcp.Tests/StructuredContentRoundTripTests.cs` (assert published-schema property names match the camelCase keys in `structuredContent`). Regression test shape: 1 added assertion to the existing round-trip test verifying `schema.properties` keys equal the keys in the `structuredContent` payload exactly. Evidence: flagged by initiative #5 executor (PR #520) as out-of-scope follow-up. |
