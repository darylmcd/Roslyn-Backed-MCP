...
| id | pri | deps | do | size | detail |
|----|-----|------|----|------|--------|
| `workspace-id-omitted-residual-recovery-coherence` | Low | — | **Residual-case recovery coherence for flipped tools** — decide elicitation-vs-schemaHint intent for the 3 `Required:false` tools, fix code or comments, pin with a residual-case test. [type: bug] [source: 2026-06-09 backlog-sweep execute] | S | items/workspace-id-omitted-residual-recovery-coherence.md |
...
| `elicitation-retry-exception-envelope` | Low | — | **Elicitation-retry exception envelope** — convert failed recovery/dispatch exceptions in `TryRecoverMissingWorkspaceIdAsync` into the standard structured `CallToolResult` error shape. [type: bug] | S | items/elicitation-retry-exception-envelope.md |
| `workspace-validation-process-kill-failure-observability` | Low | — | **Workspace-validation kill-failure observability** — add bounded non-secret observability for swallowed `Process.Kill` cleanup failures without changing validation envelopes. [type: observability] | S | items/workspace-validation-process-kill-failure-observability.md |
| `nuget-version-checker-test-wallclock-poll` | Low | — | **NuGet-checker tests: drop wall-clock poll** — expose the in-flight fetch `Task`/TCS as a test seam so tests await completion deterministically instead of 10s polling. [type: test] | S | items/nuget-version-checker-test-wallclock-poll.md |
...
| `find-implementations-corlib-root-tighten-heuristic` | Low | — | **Tighten IsCorlibAssembly heuristic** — replace the broad `StartsWith("System.")` match with a `SpecialType`/allowlist gate + non-corlib guard test. [type: refactor] | S | items/find-implementations-corlib-root-tighten-heuristic.md |
...
| `signature-help-bare-null` | Low | — | **symbol_signature_help bare null** — throw `KeyNotFoundException` on an unresolvable locator (mirror the member_hierarchy fix) instead of serializing `null`. [type: bug] | S | items/signature-help-bare-null.md |
...
| `script-execution-supervisor-cancel-catch-observability` | Low | — | **Script supervisor cancel catch** — document or log the ObjectDisposedException race on `timeoutCts.Cancel()` at hard deadline; no behavior change. [type: observability] [source: doc-audit 2026-06-11] | S | items/script-execution-supervisor-cancel-catch-observability.md |
| `legacy-bug-id-tool-descriptions` | Low | — | **Remove BUG-007/BUG-008 from tool descriptions** — replace internal bug ids in `test_discover`/`get_msbuild_properties` Description with shipped pagination/filter field names. [type: docs] [source: doc-audit 2026-06-11] | S | items/legacy-bug-id-tool-descriptions.md |
