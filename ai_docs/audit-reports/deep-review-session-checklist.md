...
Use with [`skills/mcp-server-surface-test/prompts/full.md`](../../skills/mcp-server-surface-test/prompts/full.md) — invoke via `/mcp-server-stress` (maintainer alias for `/mcp-server-surface-test --output-mode=fragments`) or directly via `/mcp-server-surface-test`. A full PASS/FAIL matrix needs an **interactive MCP session** (e.g. Cursor) with the server running (`dotnet run --project src/RoslynMcp.Host.Stdio` or global `roslynmcp`).
