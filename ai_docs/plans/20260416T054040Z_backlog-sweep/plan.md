...
| I-40 | `dr-9-7-only-partially-normalizes-whitespace` | 1 | 45000 | Fixed | in-review (PR #<n>) — confirmed NOT obsolete; `RefactoringService` gains `CollapseBlankLineRunsInRange` post-pass scoped to lines inside `[startLine, endLine]` (Roslyn `Formatter.FormatAsync` re-indents and strips trailing whitespace via the splice, but does not collapse multi-blank-line runs). |
