...
| I-37 | `dr-9-4-format-bug-003-produces-inline-itemgroup-xml` | 2 | 50000 | Fixed | in-review — `OrchestrationMsBuildXml` gains `FormatProjectXml` / `RemoveElementCleanly` / `AddChildElementPreservingIndentation`; `PackageMigrationOrchestrator` uses them; UTF-8 MemoryStream-backed `XmlWriter` replaces `ToString(SaveOptions.DisableFormatting)`. |
