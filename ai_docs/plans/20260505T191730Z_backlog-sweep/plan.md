...
- [x] **Markdown link hrefs.** Plan uses plain inline-code source citations throughout — no `[label](src/...)` links that would resolve to non-existent paths under `ai_docs/plans/<ts>_backlog-sweep/`. Verified safe against `verify-ai-docs.ps1`.
