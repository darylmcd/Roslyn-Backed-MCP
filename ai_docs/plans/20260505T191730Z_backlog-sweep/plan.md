...
| Field | Content |
|---|---|
| Status | merged (PR #516, 2026-05-06) |
...
| Field | Content |
|---|---|
| Status | merged (PR #517, 2026-05-06) |
...
| Field | Content |
|---|---|
| Status | merged (PR #526, 2026-05-06) |
...
| Scope | Production files: 3 — `src/RoslynMcp.Host.Stdio/Tools/WorkspaceTools.cs` (tool parameter + post-load warm), `src/RoslynMcp.Core/Services/IWorkspaceWarmService.cs` (opt-in docs), `src/RoslynMcp.Host.Stdio/Tools/WorkspaceWarmTools.cs` (standalone warm docs). Within Rule 3 (3/4). Test files: 1 new — `tests/RoslynMcp.Tests/Workspace/WorkspaceCachePrewarmTests.cs`; 1 existing progress test updated for the added tool dependency. |
| Tool policy | edit-only |
| Estimated context cost | 40000 |
| Risks | (1) Prewarm holds the load request open longer, but only for callers that opt in. (2) The tool must not run two warms for one load; the nullable flag makes `false` an explicit future opt-out for initiative #11. (3) Response shape changes only for `prewarm=true`, where the caller requested the additional work. |
| Validation | (a) `mcp__roslyn__compile_check` after edits. (b) New prewarm tests: `prewarm=true` warms and returns the warm block; default/false keeps the workspace cold. (c) Focused progress test update. (d) `./eng/verify-release.ps1 -Configuration Release`. |
| Performance review | Opt-in performance behavior. Focused tests assert the semantic signal: first warm after `prewarm=true` reports cold projects, and a repeat warm reports `ColdCompilationCount=0`; no default-load regression because omitted/false skips warm. |
| CHANGELOG category | Added |
| CHANGELOG entry (draft) | Added: opt-in `prewarm: true` parameter on `workspace_load` — when set and the load succeeds, runs `workspace_warm` and returns the warm result alongside the load summary. Omitted or `false` preserves the cold-load profile. Closes `workspace-cache-prewarm-on-load`. |
| Backlog sync | Close rows: [`workspace-cache-prewarm-on-load`]. Update related: `workspace-warm-default-above-50-projects` re-scoped to defaulting the new nullable `prewarm` flag above a threshold, with explicit `prewarm: false` opt-out. |
...
| Field | Content |
|---|---|
| Status | merged (PR #515, 2026-05-06) |
| Backlog rows closed | (split) `audit-deep-skill-migration` (paired with initiative #13 — row stays open until #13 ships) |
...
| Field | Content |
|---|---|
| Status | merged (PR #530, 2026-05-06) |
