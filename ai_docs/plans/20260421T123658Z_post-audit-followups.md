...
| **1.2** | Migrate **one** small group: `CodeActionTools.{ApplyCodeAction, PreviewCodeAction}` (2 methods). Lift attributes to `ICodeActionService`. Delete hand-written shims. Verify generator output matches old behavior byte-for-byte (compare `obj/Generated/.../CodeActionTools.g.cs` against pre-migration `git show HEAD:src/.../CodeActionTools.cs`). | medium | full `verify-release.ps1`; `gh pr checks` green; LLM-client smoke (manual `apply_code_action` call) |
