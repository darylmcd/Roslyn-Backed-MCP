...
| File | Role |
|------|------|
| `.mcp.json` (repo root; optional; often gitignored locally) — see [Per-Client Config](../../../README.md#per-client-config) | Registers `roslyn` with `type: stdio`, `command: roslynmcp`. Cursor loads project MCP when present. |
