...
The shipped `.claude-plugin/mcp.json` descriptor is distinct from the redundant root `.mcp.json` client registration removed by this change. The plugin layer is a pure orchestration concern — it adds no code to the C# projects. Skills reference tools by MCP name and compose them into multi-step workflows; hooks enforce safety patterns (preview before apply, compile after refactor).
