...
Use this procedure when the `/mcp-server-stress` skill (which runs the canonical prompt at `skills/mcp-server-surface-test/prompts/full.md`) is being run across multiple C# repositories and the result needs to drive backlog or release decisions.
